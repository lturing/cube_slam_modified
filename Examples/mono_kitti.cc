...
int main(int argc, char **argv)
{
    ...
    vector<double> vTimestamps;
    ORB_SLAM2::base_data_folder = argv[3];
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        filename = GetFileName(vstrImageFilenames[ni], false);
        if(im.empty())
        {
        ...
        SLAM.TrackMonocular(im, tframe, ni, filename);
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ...
    // for kitti named by date, 2011_10_03
    int flag = 0;
    string strPrefixLeft = strPathToSequence + "/data/";
    boost::filesystem::path pathObj(strPrefixLeft);
    if(!boost::filesystem::is_directory(pathObj))
    {
        // for kitti named by sequence, KITTI00
        strPrefixLeft = strPathToSequence + "/image_2/";
        flag = 1;
    }
