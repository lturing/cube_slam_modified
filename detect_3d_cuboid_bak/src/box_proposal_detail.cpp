...
void detect_3d_cuboid::detect_object_cuboid(const cv::Mat &rgb_img, const Eigen::Matrix4d &transToWolrd, Eigen::MatrixXd& obj_bbox_coors, Eigen::MatrixXd& all_lines_raw, std::vector<ObjectSet> &all_object_cuboids)
{
    // const Eigen::MatrixXd &obj_bbox_coors, Eigen::MatrixXd edges
    cv::Mat img = rgb_img.clone();
    if( img.channels() == 1 )
    ...
    int row_counter = 0;
    //Eigen::MatrixXd obj_bbox_coors;
    obj_bbox_coors.resize(10, 5);
    for(int i = 0; i < result[0].size(); i++)
    {
        // [x y w h prob]
        obj_bbox_coors(i, 0) = result[0][i].box.tl().x;
        obj_bbox_coors(i, 1) = result[0][i].box.tl().y;
        obj_bbox_coors(i, 2) = result[0][i].box.size().width; //w;
        obj_bbox_coors(i, 3) = result[0][i].box.size().height; // h;
        obj_bbox_coors(i, 4) = result[0][i].confidence;
        row_counter += 1;
    ...
    myLineDetector->detect_raw_lines(img, keylines_raw);
    myLineDetector->filter_lines(keylines_raw, keylines_out);  // remove short lines
    ...
    detect_3d_cuboid::detect_cuboid(rgb_img, transToWolrd, obj_bbox_coors, all_lines_raw, all_object_cuboids);
}
                                        ...
                                        MatrixXd& all_lines_raw, std::vector<ObjectSet> &all_object_cuboids)
{
	...
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
				...
				for (int cam_pitch_id = 0; cam_pitch_id < cam_pitch_samples.size(); cam_pitch_id++)
					for (int obj_yaw_id = 0; obj_yaw_id < obj_yaw_samples.size(); obj_yaw_id++)
					{
						...
						for (int sample_top_pt_id = 0; sample_top_pt_id < sample_top_pts.cols(); sample_top_pt_id++)
						{
							...
							{
								if (print_details)
									std::cout << "Configuration fails at corner 2, outside segment" << std::endl;
								continue;
							}
							if ((corner_1_top - corner_2_top).norm() < shorted_edge_thre)
							{
								if (print_details)
									std::cout << "Configuration fails at edge 1-2, too short" << std::endl;
								continue;
							}
							...
							for (int config_id = 1; config_id < 3; config_id++) // configuration one or two of matlab version
							{
								...
								if (config_id == 1)
								{
									...
									if (corner_4_top(1) == -1)
									{
										config_good = false;
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at corner 4, outside segment " << std::endl;
										continue;
									}
									if ((corner_1_top - corner_4_top).norm() < shorted_edge_thre)
									{
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at edge 1-4, too short " << std::endl;
										continue;
									}
									...
									{ // check inside boundary. otherwise edge visibility might be wrong
										config_good = false;
										if (print_details)
											std::cout << "Configuration "<< config_id << " fails at corner 3, outside box" << std::endl;
										continue;
									}
									if (((corner_3_top - corner_4_top).norm() < shorted_edge_thre) || ((corner_3_top - corner_2_top).norm() < shorted_edge_thre))
									{
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at edge 3-4/3-2, too short" << std::endl;
										continue;
									}
								...
								if (config_id == 2)
								{
									...
									if (corner_3_top(1) == -1)
									{
										config_good = false;
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at corner 3, outside segment" << std::endl;
										continue;
									}
									if ((corner_2_top - corner_3_top).norm() < shorted_edge_thre)
									{
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at edge 2-3, too short" << std::endl;
										continue;
									}
									// compute the last point in the top face
									...
									if (!check_inside_box(corner_4_top, Vector2d(left_x_raw, top_y_expan_distmap), Vector2d(right_x_raw, down_y_expan_distmap)))
									{
										config_good = false;
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at corner 4, outside box" << std::endl;
										continue;
									}
									if (((corner_3_top - corner_4_top).norm() < shorted_edge_thre) || ((corner_4_top - corner_1_top).norm() < shorted_edge_thre))
									{
										if (print_details)
											std::cout << "Configuration " << config_id << " fails at edge 3-4/4-1, too short" << std::endl;
										continue;
									}
									// 			      cout<<"corner_3/4   "<<corner_3_top.transpose()<<"   "<<corner_4_top.transpose()<<endl;
								}
								...
								if (corner_5_down(1) == -1)
								{
									config_good = false;
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at corner 5, outside segment" << std::endl;
									continue;
								}
								if ((corner_3_top - corner_5_down).norm() < shorted_edge_thre)
								{
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at edge 3-5, too short" << std::endl;
									continue;
								}
								...
								if (!check_inside_box(corner_6_down, expan_distmap_lefttop, expan_distmap_rightbottom))
								{
									config_good = false;
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at corner 6, outside box" << std::endl;
									continue;
								}
								if (((corner_6_down - corner_2_top).norm() < shorted_edge_thre) || ((corner_6_down - corner_5_down).norm() < shorted_edge_thre))
								{
									if (print_details)
								        std::cout << "Configuration " << config_id << " fails at edge 6-5/6-2, too short" << std::endl;
									continue;
								}
								...
								{ // might be slightly different from matlab
									config_good = false;
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at corner 7, outside box" << std::endl;
									continue;
								}
								if (((corner_7_down - corner_1_top).norm() < shorted_edge_thre) || ((corner_7_down - corner_6_down).norm() < shorted_edge_thre))
								{
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at edge 7-1/7-6, too short" << std::endl;
									continue;
								}
								...
								if (!check_inside_box(corner_8_down, expan_distmap_lefttop, expan_distmap_rightbottom))
								{
									config_good = false;
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at corner 8, outside box" << std::endl;
									continue;
								}
								...
								{
									if (print_details)
										std::cout << "Configuration " << config_id << " fails at edge 8-4/8-5/8-7, too short" << std::endl;
									continue;
								}
			...
			for (int box_id = 0; box_id < good_proposal_ids.size(); box_id++)
			{
                ...
                sample_obj->confidence = obj_bbox_coors(object_id, 4);
                sample_obj->classId = int(obj_bbox_coors(object_id, 5));
                sample_obj->trackId = int(obj_bbox_coors(object_id, 6));
	...
	if (whether_plot_final_images || whether_save_final_images)
	{
        ...
        //std::cout << "detect " << object_count << " objects" << std::endl;
	}
}
