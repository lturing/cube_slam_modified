...
bool read_obj_yolo_txt(const std::string txt_file_name, Eigen::MatrixXd &read_number_mat)
{
    // top_left_x, top_left_y, width, height, confidence, classId
    if (!std::ifstream(txt_file_name))
    {
        std::cout << "ERROR!!! Cannot read txt file " << txt_file_name << std::endl;
        return false;
...
bool read_obj_yolo_txt(const std::string txt_file_name, cv::Mat &read_number_mat)
{
    // top_left_x, top_left_y, width, height, confidence, classId
    if (!std::ifstream(txt_file_name))
    {
    ...
    std::ifstream filetxt(txt_file_name.c_str());
    std::string line;
    cv::Mat data(0, 6, CV_32F); 
    int row_counter = 0;
    while (getline(filetxt, line))
    {
        double t;
        cv::Mat aRow(1, 6, CV_32F);
        if (!line.empty())
        {
            std::stringstream ss(line);
            int colu = 0;
            ...
            {
                //read_number_mat(row_counter, colu) = t;
                aRow.at<float>(0, colu) = t;
                colu++;
            }
        }
        data.push_back(aRow.row(0));
        row_counter += 1;
    }
    filetxt.close();

    cv::Range rowsToCopy(0, row_counter);  
    data.rowRange(rowsToCopy).copyTo(read_number_mat);
    std::cout << "data.rows=" << data.rows << ", data.cols=" << data.cols << ", read_number_mat.rows=" << read_number_mat.rows << ", read_number_mat.cols=" << read_number_mat.cols << std::endl;
    return true;
}
