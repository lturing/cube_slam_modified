...
void cameratoimage(Eigen::Vector3d& point_cameras, Eigen::Matrix3d& Kalib, Eigen::Vector2d& points)
{
    /*
    point_cameras: (3, N), N points on X(right)-Y(down)-Z(front) camera plane
    projection: (3, 3), projection matrix
    points: (2, N), N points on X-Y image plane
    */
    Vector3d corners = Kalib * point_cameras; // 3 * N
...
bool is_before_clip_plane_camera(Eigen::Vector3d& points_camera, double cam_near_clip)
{
    /*
    points_camera: X(right)-Y(down)-Z(front) camera coordinate
    cam_near_clip(0.15): scalar, the near projection plane
...
void get_3d_bbox_vertex(Eigen::Matrix3d& Kalib, Eigen::Matrix3Xd& points3d, Eigen::Matrix2Xd& points2d, double cam_near_clip)
{
    /*
    '''Get 3D bbox vertex in camera coordinates
    Input:
        cam_calib: (3, 3), projection matrix
    ...
    */
    ...
    std::vector<std::vector<int> > lineorder { 
                                            {1, 2, 6, 5},  
                                            {2, 3, 7, 6}, 
                                            {3, 4, 8, 7},  
                                            {4, 1, 5, 8},
    ...
    Eigen::Vector3d cam_dir(0, 0, 1);
    Eigen::Vector3d center_pt = cam_dir * cam_near_clip;

    points2d.resize(2, 1);
    int col_counter = 0;

    //std::cout << "points3d.rows=" << points3d.rows() << ", points3d.col=" << points3d.cols() << std::endl;
    for (int i = 0; i < lineorder.size(); i++)
    {
        for (int j = 0; j < 4; j++)
        {
            int index1 = lineorder[i][j] - 1;
            int index2 = lineorder[i][(j+1) % 4] - 1;
            Vector3d p1(points3d(0, index1), points3d(1, index1), points3d(2, index1));
            Vector3d p2(points3d(0, index2), points3d(1, index2), points3d(2, index2));
            bool before1 = is_before_clip_plane_camera(p1, cam_near_clip);
            bool before2 = is_before_clip_plane_camera(p2, cam_near_clip);

            Vector3d inter_point;
            get_intersect_point(center_pt, cam_dir, p1, p2, inter_point);
            ...
            Eigen::Vector2d cp1;
            Eigen::Vector2d cp2;
            cameratoimage(vp1, Kalib, cp1);
            cameratoimage(vp2, Kalib, cp2);

            if ((col_counter + 2) >= points2d.cols())
            {
                points2d.conservativeResize(2, points2d.cols() * 3);
            }
            points2d.col(col_counter) = cp1;
            col_counter++;
            points2d.col(col_counter) = cp2;
            col_counter++;
        }
    }
    points2d.conservativeResize(2, col_counter);
}


void get_intersect_point(Eigen::Vector3d& center_pt, Eigen::Vector3d& cam_dir, Eigen::Vector3d& vertex1, Eigen::Vector3d& vertex2, Eigen::Vector3d& inter_point)
{
    double k_up = abs(cam_dir.transpose() * (vertex1 - center_pt));
    double k_down = abs(cam_dir.transpose() * (vertex1 - vertex2));
    double k;
    if (k_up > k_down)
        k = 1;
    else
...
bool get2dBox(Eigen::Matrix3d& Kalib, Eigen::Matrix3Xd& points3d, Eigen::Matrix2Xi& points2d, Eigen::Vector4d& box_size, int img_width, int img_height, double cam_near_clip)
{
    Eigen::Matrix2Xd cam_point;
    get_3d_bbox_vertex(Kalib, points3d, cam_point, cam_near_clip);
    if (cam_point.cols() < 2)
        return false;
    
    double x_min = cam_point.row(0).minCoeff();
    double x_max = cam_point.row(0).maxCoeff();
    double y_min = cam_point.row(1).minCoeff();
    double y_max = cam_point.row(1).maxCoeff();
    x_min = (x_min > 0 ? x_min : 0);
    x_max = (x_max < img_width ? x_max : img_width);

    y_min = (y_min > 0 ? y_min : 0);
    y_max = (y_max < img_height ? y_max : img_height);

    if (x_min >= x_max || y_min >= y_max)
        return false;
    
    Eigen::Vector4d bsize(x_min, y_min, (x_max - x_min), (y_max - y_min));
    box_size = bsize;
    points2d = cam_point.cast<int>();
    return true;

}


void computeboxes(cuboid* raw_cuboid, Eigen::Matrix4d& Twc, Eigen::Matrix3d& Kalib, int img_width, int img_height, double cam_near_clip)
{
    ...
    Matrix3d rotation = euler_zyx_to_rot<double>(0.0, raw_cuboid->rotY, 0.0);
    Matrix3d scale_mat = raw_cuboid->scale.asDiagonal();
    Matrix4d T = Matrix4d::Identity();
    T.topLeftCorner<3, 3>() = rotation * scale_mat;
    T.col(3).head(3) = raw_cuboid->pos;

    points3d = homo_to_real_coord<double>(T * real_to_homo_coord<double>(corners_body));
    bool flag = get2dBox(Kalib, points3d, raw_cuboid->box_corners_2d, raw_cuboid->rect_detect_2d, img_width, img_height, cam_near_clip);
    raw_cuboid->box_corners_3d_world = homo_to_real_coord<double>(Twc * real_to_homo_coord<double>(points3d));
}
...
void computeBoxCorner(double rotY, Eigen::Vector3d& pos, Eigen::Vector3d& scale, int image_width, int image_height, Eigen::Matrix4d& Twc, Eigen::Matrix3d& Kalib, Eigen::Vector4d& rect_detect_2d, Eigen::Matrix2Xi& box_2d, Eigen::Matrix3Xd& box_3d)
{
    box_2d.resize(2, 8);
    box_3d.resize(3, 8);
    //Matrix3d rotation = euler_zyx_to_rot<double>(0.0, 0.0, rotY);
    Matrix3d rotation = euler_zyx_to_rot<double>(0.0, rotY, 0.0);
    MatrixXd corners_body;
    corners_body.resize(3, 8);
    /*
    corners_body << 0.5, 0.5, -0.5, -0.5, 0.5, 0.5, -0.5, -0.5,
    ...
    */
    ...
    //std::cout << "rotY=" << rotY << ", rotation=" << rotation << ", pos=" << pos << std::endl;
    ...
    box_3d = homo_to_real_coord<double>(T * real_to_homo_coord<double>(corners_body));
    MatrixXd corners = Kalib * box_3d; // 3 * 8
    //std::cout << "box_3d=" << box_3d << ", corners=" << corners << std::endl;
    corners.row(0) = corners.row(0).array() / corners.bottomRows(1).array();
    corners.row(1) = corners.row(1).array() / corners.bottomRows(1).array();
    //std::cout << "corners=" << corners << std::endl;
    //std::cout << "rect_detect_2d=" << rect_detect_2d << std::endl;
    double x_min = corners.row(0).minCoeff();
    double x_max = corners.row(0).maxCoeff();
    double y_min = corners.row(1).minCoeff();
    double y_max = corners.row(1).maxCoeff();
    Eigen::Vector4d box_size(x_min, y_min, (x_max - x_min), (y_max - y_min));
    //std::cout << "box_size=" << box_size << std::endl;
    rect_detect_2d = box_size;
    corners.array() += 0.5;
    box_2d = corners.topLeftCorner<2, 8>().cast<int>();
    box_3d = homo_to_real_coord<double>(Twc * real_to_homo_coord<double>(box_3d));
}
