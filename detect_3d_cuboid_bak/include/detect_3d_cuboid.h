...
class detect_3d_cuboid
{
    public:
      ...
      void detect_object_cuboid(const cv::Mat &rgb_img, const Eigen::Matrix4d &transToWolrd, Eigen::MatrixXd& obj_bbox_coors, Eigen::MatrixXd& all_lines_raw, std::vector<ObjectSet> &all_object_cuboids);
      ...
      std::shared_ptr<Detector_ONNXRUNTIME> myDetectorOnnxRun;
      std::shared_ptr<Detector_OpenCV_DNN> myDetectorOnnxCV;
      std::shared_ptr<line_lbd_detect> myLineDetector;
