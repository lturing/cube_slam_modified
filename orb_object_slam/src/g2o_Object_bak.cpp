...
Vector4d cuboid::projectOntoImageBbox(SE3Quat &campose_cw, Matrix3d &Kalib)
{
    ...
    Matrix3Xd corners_camera = homo_to_real_coord<double>(campose_cw.to_homogeneous_matrix()  * similarityTransform() * real_to_homo_coord<double>(corners_body));
    Eigen::Vector4d box_size;
    bool flag = get2dBox(Kalib, corners_camera, box_size, ORB_SLAM2::image_width, ORB_SLAM2::image_height, cam_near_clip);
    if (flag)
        return Vector4d(box_size(0) + box_size(2) / 2.0, box_size(1) + box_size(3) / 2.0, box_size(2), box_size(3));
    return Vector4d(-100, -100, -100, -100);
}
...
SE3Quat exptwist_norollpitch(const Vector6d &update)
{
    ...
    // #todo, default ground
    Matrix3d R;
    if (ORB_SLAM2::build_worldframe_on_ground)
    {
    ...
    {
        R << cos(omega(1)), 0, sin(omega(1)),
             0, 1, 0,
             -sin(omega(1)), 0, cos(omega(1));
    }
...
void EdgeSE3CuboidFixScaleProj::computeError()
{
    ...
    /*
    if (rect_project(0) < -10)
        _error = Vector4d(0.0, 0.0, 0.0, 0.0);
    else 
        _error = rect_project - _measurement;
    */
}
...
{
    const VertexCuboid *cuboidVertex = dynamic_cast<const VertexCuboid *>(_vertices[0]); // object pose to world
...
{
    const VertexCuboidFixScale *cuboidVertex = dynamic_cast<const VertexCuboidFixScale *>(_vertices[0]); // object to world
