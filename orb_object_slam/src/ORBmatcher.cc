...
int ORBmatcher::SearchByTrackingHarris(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    if (LastFrame.mImGray.rows == 0 || LastFrame.mImGray.cols == 0)
    {
        std::cout << "Matcher feature tracking  zero image size" << std::endl;
        return 0;
    }
    ...
    cv::calcOpticalFlowPyrLK(LastFrame.mImGray, CurrentFrame.mImGray, lastobjpts, currobjpts, status, err, cv::Size(21, 21), 3); // could also do a Backward optical flow.
...
int ORBmatcher::SearchByTracking(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    cv::calcOpticalFlowPyrLK(LastFrame.mImGray, CurrentFrame.mImGray, lastobjpts, currobjpts, status, err, cv::Size(21, 21), 3);
        ...
        for (size_t i = 0; i < status.size(); i++)
        {
            if (status[i])
            {
                ...
                if (last_obj_ind >= LastFrame.numobject)
                    std::cout << "Matcher Found large object id " << last_obj_ind << " " << LastFrame.numobject << std::endl;
                ...
                {
                    std::cout << "Matcher Found bad object id " << last_obj_ind << std::endl;
                }
            }
        }
