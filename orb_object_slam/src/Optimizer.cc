        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (pMP)
            {
                ...
                if (pFrame->mvuRight[i] < 0)
                {
                    ...
                    {
                        cout << "Found static map point, dynamic 2d feature" << std::endl;
                        continue;
                    }
...
void Optimizer::LocalBACameraPointObjects(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool fixCamera, bool fixPoint)
{
    ...
    std::cout << "LocalBAPointObjects num of local points / objects / frames / fixed frames: " << lLocalMapPoints.size() << " " << lLocalMapObjects.size() << " " << lLocalKeyFrames.size() << " " << lFixedCameras.size() << std::endl;
...
#define ObjectFixScale // use when scale is provided and fixed. such as KITTI
    ...
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
    {
        ...
        if (scene_unique_id == kitti)
            ...
            vObject->fixedscale = pMObject->cube_meas.scale;
        else
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++) // no need to optimize all objects...., use local KF's map objects?
        {
            ...
            if (1) // an object connected to many fixed points. optimize only object
            {
                ...
                if (e->object_points.size() > 10)
                {
                    ...
                    {
                        e->max_outside_margin_ratio = 2;
                        e->prior_object_half_size = pMO->cube_meas.scale;
                    }
                    optimizer.addEdge(e);
                }
            }
...
void Optimizer::LocalBACameraPointObjectsDynamic(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool fixCamera, bool fixPoint)
{
    ...
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
    {
        ...
        for (unordered_map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
        {
            ...
            if ((pKF->mTimeStamp - pKFi->mTimeStamp) < 8.0 && pKFi->mnBALocalForKF != pKF->mnId && pKFi->mnBAFixedForKF != pKF->mnId) // not been added to lLocalKeyFrames, and lFixedCameras!
            {
                pKFi->mnBAFixedForKF = pKF->mnId;
                if (!pKFi->isBad())
                    lFixedCameras.push_back(pKFi);
...
#define ObjectFixScale
    ...
    // Set MapObject vertices
    long int maxObjectid = 0;
    // #todo, check objfixscale, whether build_on_ground
    ...
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
    {
        ...
        for (unordered_map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
        {
            ...
            {
                std::cout << "BA not found frame object pose!!! objID " << pMObject->mnId << " frameid " << pKFi->mnFrameId << std::endl;
    ...
    if (ba_dyna_pt_obj_cam)
    {
        for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
        {
            ...
            // #todo, change objfixscale when dataset changing
    ...
    vector<g2o::EdgeObjectMotionV2 *> allmotionedgesV2;
    vector<g2o::EdgeObjectMotionV3 *> allmotionedgesV3;
    ...
    if (ba_dyna_obj_velo)
    {
        if (0)
        {
            ...
            const float thHuberObjectVeloc = sqrt(4.0);
            for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
            {
                MapObject *pMObject = *lit;
                ...
                {
                    //std::cout << "not enough kf observed by object " << pMObject->bundle_vertex_ids.size() << std::endl;
                    continue;
                }
                ...
                for (size_t i = 0; i < objkfs.size(); i++)
                {
                    ...
                    if (!obskf->isBad() && pMObject->bundle_vertex_ids.count(obskf))
                    {
                        ...
                        else // add velocity edge
                        {
                            ...
                            g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
                            e->setRobustKernel(rk);
                            rk->setDelta(thHuberObjectVeloc);                            
                            optimizer.addEdge(e);
        ...
        else
        {
            ...
            inv_sigma(2) = inv_sigma(2) * 5.0; //0.1 angle error is usually much smaller compared to position.
            inv_sigma = inv_sigma * object_velocity_BA_weight;
            Eigen::Matrix4d object_velocity_sigma = inv_sigma.cwiseProduct(inv_sigma).asDiagonal();
            ...
            for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
            {
                ...
                for (size_t i = 0; i < objkfs.size(); i++)
                {
                    ...
                    if (!obskf->isBad() && pMObject->bundle_vertex_ids.count(obskf))
                    {
                        ...
                        {
                            g2o::EdgeObjectMotionV2 *e = new g2o::EdgeObjectMotionV2();
                            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pMObject->bundle_vertex_ids[prevgoodobsframe])));
                            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pMObject->bundle_vertex_ids[obskf])));
                            e->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(vVelocity));
                            e->delta_t = obskf->mTimeStamp - prevgoodobsframe->mTimeStamp;
                            ...
                            //std::cout << "error=" << e->computeError_debug() << std::endl;
                            optimizer.addEdge(e);

                            prevgoodobsframe = obskf;
                            lastgoodobsframe = obskf;
                            allmotionedgesV2.push_back(e);
                        }
                    }
                }
        ...
        if (1)
        {
            ...
            Eigen::Vector3d inv_sigma;
            inv_sigma.setOnes();
            inv_sigma(0) = inv_sigma(0) * 5.0;
            inv_sigma = inv_sigma * 2.0 ; //object_velocity_BA_weight;
            Eigen::Matrix3d object_velocity_sigma = inv_sigma.cwiseProduct(inv_sigma).asDiagonal();
            ...
            for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
            {
                ...
                int id = pMObject->mnId + maxIdTillPoint + 1;
                g2o::VelocityPlanarVelocity *vVelocity = dynamic_cast<g2o::VelocityPlanarVelocity *>(optimizer.vertex(id));
                if (vVelocity == NULL)
                    continue; 
                int valid_observed_frames = 0;
                ...
                //for a dynamic object, create each vertex for each observed frame
                vector<KeyFrame *> objkfs = pMObject->GetObserveFramesSequential();
                KeyFrame* frame_1 = NULL;
                KeyFrame* frame_2 = NULL;
                for (size_t i = 0; i < objkfs.size(); i++)
                {
                    KeyFrame *obskf = objkfs[i];
                    if (!obskf->isBad() && pMObject->bundle_vertex_ids.count(obskf))
                    {
                        ...
                        else if (frame_2 == NULL)
                        {
                            frame_2 = obskf;
                        }
                        else // add velocity edge
                        {
                            g2o::EdgeObjectMotionV3 *e = new g2o::EdgeObjectMotionV3();
                            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pMObject->bundle_vertex_ids[frame_1])));
                            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pMObject->bundle_vertex_ids[frame_2])));
                            e->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pMObject->bundle_vertex_ids[obskf])));
                            e->setVertex(3, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));
                            e->delta_t_21 = frame_2->mTimeStamp - frame_1->mTimeStamp;
                            e->delta_t_32 = obskf->mTimeStamp - frame_2->mTimeStamp;
                            //e->delta_t = obskf->mTimeStamp - prevgoodobsframe->mTimeStamp;
                            //           cout<<"add obj id for frames:   "<<pMObject->mnId<<"   "<<prevgoodobsframe->mnId<<"   "<<obskf->mnId<<"    delta "<<e->delta_t <<endl;
                            //           cout<<"obskf->mTimeStamp   "<<obskf->mTimeStamp<<"   "<<prevgoodobsframe->mTimeStamp<<endl;
    ...
    std::cout << "BA edges point-cam: " << vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size() << " object-cam: " << vpEdgesCameraObject.size()
              << " pt-obj-cam: " << vpEdgesCameraPointObject.size() << " pt-obj: " << vpEdgesPointObject.size() << " obj-vel: " << (allmotionedges.size() + allmotionedgesV2.size()) 
              << " obj-obj: " << allmotionedgesV3.size() << std::endl;
