...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, string filename)
{
    if (mSensor != STEREO)
    {
    ...
    return mpTracker->GrabImageStereo(imLeft, imRight, timestamp, filename);
}
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, string filename)
{
    ...
    return mpTracker->GrabImageRGBD(im, depthmap, timestamp, filename);
}

cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, int msg_seq_id, string filename)
{
    if (mSensor != MONOCULAR)
    {
    ...
    return mpTracker->GrabImageMonocular(im, timestamp, msg_seq_id, filename);
}
