...
std::shared_ptr<Segmentor_ONNXRUNTIME> Frame::mySegOnnxRun;
std::shared_ptr<Segmentor_OpenCV_DNN> Frame::mySegOnnxCV;
std::shared_ptr<byte_track::BYTETracker> Frame::mpByteTracker;
...
Frame::Frame(const Frame &frame)
    : mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
      ...
      mvLevelSigma2(frame.mvLevelSigma2), mvInvLevelSigma2(frame.mvInvLevelSigma2), msfilename(frame.msfilename)
{
    ...
    if (frame.raw_all_obj2d_bbox.rows() > 0)
        raw_all_obj2d_bbox = frame.raw_all_obj2d_bbox;
}

// for stereo
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat& imLeftBGR, const double &timeStamp, ORBextractor *extractorLeft, ORBextractor *extractorRight, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, string filename)
    : mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft), mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), msfilename(filename)
{
    // Frame ID
    mnId = nNextId++;
    ...
    if (whether_detect_object || whether_dynamic_object)
    {
        ...
        mImBGR = imLeftBGR.clone();
    }

    if (whether_dynamic_object)
    {
        SegentRemoveDynamicPoints();
    }
...
// for RGBD
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat& imBGR, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, string filename)
    : mpORBvocabulary(voc), mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
      mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), msfilename(filename)
{
    // Frame ID
    mnId = nNextId++;
...
// for monocular
Frame::Frame(const cv::Mat &imGray, const cv::Mat& imBGR, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef, const float &bf,
             const float &thDepth, string filename)
    : mpORBvocabulary(voc), mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
...
void Frame::SegentRemoveDynamicPoints()
{
    if (1)
    {
        objmask_img = cv::Mat(mImGray.rows, mImGray.cols, CV_8UC1, cv::Scalar(0));
        ...
        for (int i = 0; i < result[0].size(); ++i) 
        {
            ...
            {
                objmask_img(result[0][i].box).setTo(cv::Scalar(classId + 1), result[0][i].boxMask);
            }
        }

        std::vector<byte_track::STrackPtr> output_stracks = mpByteTracker->update(result[0]);
        raw_all_obj2d_bbox.resize(result[0].size(), 7);
        for (unsigned long i = 0; i < output_stracks.size(); i++)
        {
            raw_all_obj2d_bbox(i, 0) = output_stracks[i]->tlwh[0];
            raw_all_obj2d_bbox(i, 1) = output_stracks[i]->tlwh[1];
            raw_all_obj2d_bbox(i, 2) = output_stracks[i]->tlwh[2];
            raw_all_obj2d_bbox(i, 3) = output_stracks[i]->tlwh[3];
            raw_all_obj2d_bbox(i, 4) = output_stracks[i]->score;
            raw_all_obj2d_bbox(i, 5) = output_stracks[i]->classID;
            raw_all_obj2d_bbox(i, 6) = output_stracks[i]->track_id;
        }
    ...
    else 
    {
        ...
        std::string pred_mask_img_name = base_data_folder + "/" + seg_folder + "/" + msfilename + ".png";
        objmask_img = cv::imread(pred_mask_img_name, cv::IMREAD_UNCHANGED); // uint8  sometimes read image might take long time....

        std::string pred_info_name = base_data_folder + "/" + seg_folder + "/" + msfilename + ".txt";
        read_obj_yolo_txt(pred_info_name, raw_all_obj2d_bbox);
    }
    ...
    if (remove_dynamic_features) // directly delete dynamic region features
    {
        ...
        //std::cout << "Delete dynamic feature points " << mvKeys.size() - mvKeys_cp.size() << ", left " << mvKeys_cp.size() << " points" << std::endl;
