...
void EdgeObjectMotionV3::computeError()
{
    const VertexCuboidFixScale *cuboid_1 = dynamic_cast<const VertexCuboidFixScale *>(_vertices[0]);   // object to world pose
    const VertexCuboidFixScale *cuboid_2 = dynamic_cast<const VertexCuboidFixScale *>(_vertices[1]);     // object to world pose
    const VertexCuboidFixScale *cuboid_3 = dynamic_cast<const VertexCuboidFixScale *>(_vertices[2]);     // object to world pose
    const VelocityPlanarVelocity* velocityVertex = dynamic_cast<const VelocityPlanarVelocity *>(_vertices[3]); // object to world pose

    if (cuboid_1 == nullptr || cuboid_2 == nullptr || cuboid_3 == nullptr || velocityVertex == nullptr)
        cout << "Bad casting when compute Edge motion error!!!!!!!!!!!!!" << endl;

    // predict motion x y yaw and compute measurement.
    SE3Quat pose_1 = cuboid_1->estimate().pose;
    SE3Quat pose_2 = cuboid_2->estimate().pose;
    SE3Quat pose_3 = cuboid_3->estimate().pose;

    double yaw_1 = pose_1.toXYZPRYVector()(5);
    double yaw_2 = pose_2.toXYZPRYVector()(5);
    ...
    Vector3d trans_2_back_pred = pose_1.translation() + (velocity(0) * delta_t_21 - vehicle_length * 0.5) * Vector3d(cos(yaw_1), sin(yaw_1), 0);
    double yaw_2_pred = yaw_1 + tan(velocity(1)) * delta_t_21 / vehicle_length * velocity(0);
    // as mentioned in paper: my object frame is at the center. the motion model applies to back wheen center. have offset.
    Vector3d trans_2_pred = trans_2_back_pred + vehicle_length * 0.5 * Vector3d(cos(yaw_2_pred), sin(yaw_2_pred), 0);

    Vector3d trans_3_back_pred = pose_2.translation() + (velocity(0) * delta_t_32 - vehicle_length * 0.5) * Vector3d(cos(yaw_2), sin(yaw_2), 0);
    double yaw_3_pred = yaw_2 + tan(velocity(1)) * delta_t_32 / vehicle_length * velocity(0);
    // as mentioned in paper: my object frame is at the center. the motion model applies to back wheen center. have offset.
    Vector3d trans_3_pred = trans_3_back_pred + vehicle_length * 0.5 * Vector3d(cos(yaw_3_pred), sin(yaw_3_pred), 0);

    Vector3d p21 = (pose_2.translation() - pose_1.translation()).normalized();
    Vector3d p32 = (pose_3.translation() - pose_2.translation()).normalized();

    _error = Vector3d(p21.dot(p32), trans_2_pred(2) - pose_2.translation()[2], trans_3_pred(2) - pose_3.translation()[2]);

}

Vector3d EdgeObjectMotionV3::computeError_debug()
{
    computeError();
    return _error;
}
...
void EdgeObjectMotionV2::computeError()
{
    ...
    _error = Vector4d(poseto.translation()[0], poseto.translation()[1], yaw_to, poseto.translation()[2]) - Vector4d(trans_pred(0), trans_pred(1), yaw_pred, posefrom.translation()[2]);
...
Vector4d EdgeObjectMotionV2::computeError_debug()
{
    computeError();
    return _error;
}
