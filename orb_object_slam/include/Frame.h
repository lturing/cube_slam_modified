...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat& imLeftBGR, const double &timeStamp, ORBextractor *extractorLeft, ORBextractor *extractorRight, ORBVocabulary *voc,
          cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, string filename="");

    // Constructor for RGB-D cameras. extract keypoints, descriptors
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat& imBGR, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc,
          cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, string filename="");

    // Constructor for Monocular cameras. extract keypoints, descriptors
    Frame(const cv::Mat &imGray, const cv::Mat& imBGR, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef,
          const float &bf, const float &thDepth, string filename="");
    ...
    void SegentRemoveDynamicPoints();
...
public:
    ...
    static std::shared_ptr<Segmentor_ONNXRUNTIME> mySegOnnxRun;
    static std::shared_ptr<Segmentor_OpenCV_DNN> mySegOnnxCV;
    static std::shared_ptr<byte_track::BYTETracker> mpByteTracker;

    Eigen::MatrixXd raw_all_obj2d_bbox;
    //cv::Mat raw_all_obj2d_bbox;
    cv::Mat mImGray;
    cv::Mat mImBGR;
