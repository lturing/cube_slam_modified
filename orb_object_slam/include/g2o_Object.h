...
class EdgeObjectMotionV3 : public BaseMultiEdge<3, Vector3d>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
	EdgeObjectMotionV3() { 
	...
	double delta_t_21; // to_time - from_time   positive   velocity needs time
    double delta_t_32;
	double get_error_norm(bool print_details = false);
};
...
// dynamic object motion constraints.
class EdgeObjectMotionV2 : public BaseMultiEdge<4, Vector4d>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
	EdgeObjectMotionV2() { resize(3); };

	virtual bool read(std::istream &is) { return true; };
	virtual bool write(std::ostream &os) const { return os.good(); };

	void computeError();
	Vector4d computeError_debug();
