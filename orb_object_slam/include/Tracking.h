...
public:
    ...
    bool read_bool(const cv::FileNode &node, bool &value, bool default_value=false);
    ...
    cv::Mat GrabImageMonocular(const cv::Mat &im, const double &timestamp, int msg_seq_id = -1, string filename="");
...
public:
    ...
    unordered_map<std::string, Eigen::MatrixXd> all_offline_object_cubes;
    std::shared_ptr<detect_3d_cuboid> detect_cuboid_obj;
    ...
    void Reset();
    std::shared_ptr<byte_track::BYTETracker> mpByteTracker;
